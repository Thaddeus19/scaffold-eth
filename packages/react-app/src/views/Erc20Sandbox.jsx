...
function Erc20Sandbox({ address, localContracts, mainnetProvider }) {
  ...
  const [updateFormFunction, setUpdateFormFunction] = useState(
  ...
  const makeCall = async (callName, contract, args) => {
    if (contract[callName]) {
      ...
      console.warn(
        `Attempt to call ${callName} on ${contract} but that method doesn't exist.`,
      );
    }
  };
  ...
  const getErc20Info = async () => {
    if (selectedContract) {
      let _totalSupply = await makeCall(
        "totalSupply",
        localContracts[selectedContract],
      );
      let _decimals = await makeCall(
        "decimals",
        localContracts[selectedContract],
      );
  ...
  const getAddressInfo = async values => {
    ...
    if (selectedContract) {
      ...
      let _allowanceOf = await makeCall(
        "allowance",
        localContracts[selectedContract],
        [address, values.address],
      );
      let _myAllowanceOn = await makeCall(
        "allowance",
        localContracts[selectedContract],
  ...
  const executeUpdateFunction = async values => {
    ...
    let selectedFunction = values.function
      ? values.function
      : defaultUpdateFunction;
    ...
    <Card style={{ maxWidth: 600, width: "100%", margin: "auto" }}>
      ...
      {selectedContract ? (
        <>
          ...
          {selectedContract &&
          localContracts[selectedContract]["mintTokens"] ? (
            <Button
              onClick={async () => {
                let result = await makeCall(
                  "mintTokens",
          ...
          {selectedContract &&
          localContracts[selectedContract]["burnTokens"] ? (
            <Button
              onClick={() => {
          ...
          {selectedContract &&
          localContracts[selectedContract]["outstandingTokens"] ? (
            <Button
              onClick={async () => {
                let result = await makeCall(
                  "outstandingTokens",
          ...
          {selectedContract &&
          localContracts[selectedContract]["mintOutstandingTokens"] ? (
            <Button
              onClick={() => {
                makeCall(
